  ...
  Take(id) {
    ...
    if (id == 7) {
      ...
      this.camera.getPicture(this.cameraOptions).then((imageData7) => {
        ...
        watermark([this.photo7])
          .image(watermark.text.upperLeft('วันที่:' + this.myDate + 'สถานที่' +
            this.CustomerName + ' ' + 'สินค้า:' + this.ItemsName + ' ' + 'S/N:' + ' ' + this.SerialNo,
            '7px Arial', '#000000', 0.5))
          .then(img => {
            this.photo7 = img.src;
          });
        let params = {
